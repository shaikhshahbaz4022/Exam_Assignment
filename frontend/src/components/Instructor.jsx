...
const Instructor = () => {
    ...
    const handleSubmit=()=>{
        ...
        }).then(res=>res.json()).then((res)=>{alert("Questions Added Successfully");window.location.reload()}).catch(e=>console.log(e))
    }
    ...
    useEffect(()=>{
axios.get(`${process.env.REACT_APP_URL}/exam/allusers`,{
  headers:{
    Authorization:`Bearer ${user.token}`
  }
}).then((res)=>{setSingleName(res.data)}).catch(e=>console.log(e))
    },[])
...
}).then(res=>res.json()).then((res)=>{alert(res.msg)}).then(e=>console.log(e))
    }
  ...
  return (
   ...
   <Flex gap={'5vh'} direction={{md:'row',base:'column'}}>
    ...
    <Modal isOpen={isOpen} onClose={onClose}>
        ...
        <ModalContent>
          ...
          <ModalBody>
           ...
           <Text>Title</Text>
            ...
            <Flex align={'center'} gap={'2vh'}>
           <Text>Question</Text>
            <Input placeholder='Enter Question' onChange={(e)=>{handleChange(e)}}/>
            </Flex>
           ...
           <Text>Start Date</Text>
            <Input placeholder='Enter ' type='datetime-local' onChange={(e)=>setStartDate(e.target.value)} />
            </Flex>
            <Flex align={'center'} gap={'2vh'}>
           <Text>End Date</Text>
            <Input placeholder='Enter ' type='datetime-local'  onChange={(e)=>setEndDate(e.target.value)}/>
     ...
     <Grid templateColumns={{lg:'repeat(2,1fr)',base:'repeat(1,1fr)'}} gap={'5vh'}>
     {store?.map((item,i)=>{
            return <Box key={i} border={'1px solid gray'} p={'2vh'}>
               ...
               <Text>Ans{i+1}{". "}{item?.answer}</Text>
                ...
                <Button onClick={()=>handleToggle(item._id,i)}>Checked</Button>
            </Box>
        })}
     </Grid>
